...
switch($_GET['mode']):
	case 'selectAllCategory':
		$strsql="SELECT * FROM productcategory";
...
switch($_POST['mode']):
	case 'insert':
		$strsql="INSERT INTO productcategory (category,status)VALUES('".$_POST['category']."','".$_POST['status']."')";
	...
	break;
	case 'update':
		$strsql="UPDATE productcategory SET category='".$_POST['category']."',status='".$_POST['status']."' WHERE categoryID='".$_POST['categoryID']."'";		
		if($database->execute($strsql)){
		echo 'true';
		}else{
		echo 'false';
	...
	break;
	case 'delete':
		$strsql="DELETE FROM productcategory WHERE categoryID='".$_POST['categoryID']."'";	
		if($database->execute($strsql)){
		echo 'true';
		}else{
		echo 'false';
	...
	break;
	case 'getLastID':
		$strsql="SELECT AUTO_INCREMENT AS lastID FROM information_schema.tables WHERE table_name='productcategory' AND table_schema = 'inventorysystem'";
...
var script= new function() {
	...
	this.initial=function(){
		...
		$("#gridTable").kendoGrid({
	        dataSource: {
	        	transport: {read: "productcategory.php?mode=selectAllCategory"},	            
	            dataType: "json",
	            autoSync: true,
	            pageSize: 5,
	            schema: {
	        ...
	        columns: [ 
	        	{field: "categoryID",title: "ID",width: 60, type: "number"},
	...
	this.eventhandle=function(){
		...
		if($("#edit").hasClass("k-state-disabled"))return;
			...
			if(selectedItem==null){alert('Please select record to edit.');return;}
				...
				$('#categoryID').val(selectedItem.categoryID);
		...
		if($("#delete").hasClass("k-state-disabled"))return;
			...
			if(confirm('Do you want to delete this record?')){
				if(ajax('productcategory.php',({mode:'delete',categoryID:selectedItem.categoryID}),false)=='true'){
					script.clearform();
				}
			}							
		});		
	...
	this.clearform=function(){
		...
		//$('#scriptForm')[0].reset();
		var lastID=ajax('productcategory.php',({mode:'getLastID'}),false);
...
<div id="main">
	...
	<div class="wrapper">
		...
		<div id="content">
			<div class="box">
				<form id="scriptForm" action="productcategory.php" method="post" name="scriptForm">
					<input id="mode" name="mode" type="hidden" value="insert" />
					<?php include_once('profileheader.php'); ?>
					<fieldset class="k-content">
					<legend style="color: #37b2d1">Manage product category</legend>
					<fieldset>
					...
					<div>
						<div>
							<label>ID :</label>
							<input id="categoryID" class="input" name="categoryID" readonly title="Click add new to gen new ID" type="text">
							<span class="k-invalid-msg" data-for="categoryID">
							</span></div>
						<div>
							<label>Category :</label>
							<input id="category" class="input" name="category" required="" title="Enter category" type="text">*
